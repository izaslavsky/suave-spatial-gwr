...
# ── All cached functions at module scope ──────────────────────────────────────
...
def make_feature_map(geojson_str: str, geom_type: str, name_cols: list):
    ...
    try:
        ...
        pts    = [shape(f["geometry"]).centroid for f in data["features"] if f["geometry"]]
    ...
    if geom_type == "Point":
        ...
        props   = [f["properties"] for f in data["features"][:1]]
        tip_fld = list(props[0].keys())[:4] if props else []
        folium.GeoJson(
            data,
            tooltip=folium.GeoJsonTooltip(fields=tip_fld) if tip_fld else None,
        ).add_to(m)
    return m, center
...
def make_residual_map(geojson_str: str, geom_type: str, center: list):
    ...
    res_vals = [f["properties"].get("residual#number") for f in data["features"]
                if f["properties"].get("residual#number") is not None]
    ...
    if geom_type == "Point":
        ...
        rows = [(i, f["properties"]["residual#number"])
                for i, f in enumerate(data["features"])
                if "residual#number" in f["properties"]]
        folium.Choropleth(
            geo_data=data,
            data=pd.DataFrame(rows, columns=["idx", "residual#number"]),
...
def make_lisa_map(geojson_str: str, geom_type: str, center: list):
    ...
    li_vals = [f["properties"].get("local_I#number") for f in data["features"]
               if f["properties"].get("local_I#number") is not None]
    ...
    if geom_type == "Point":
        ...
        rows = [(i, f["properties"]["local_I#number"])
                for i, f in enumerate(data["features"])
                if "local_I#number" in f["properties"]]
        folium.Choropleth(
            geo_data=data,
            data=pd.DataFrame(rows, columns=["idx", "local_I#number"]),
...
# ── Session state init ────────────────────────────────────────────────────────
for _k in ("gwr_results", "gwr_df", "bw", "dependent_var", "independent_vars",
...
if st.button("▶️ Run GWR") and dependent_var and independent_vars:
    with st.spinner("Fitting GWR model…"):
        ...
        for col, vals in zip(coeff_cols, gwr_results.params.T):
            gwr_df[f"{col}#number"] = vals
...
if st.session_state.gwr_results is not None:
    ...
    with col_a:
        st.download_button("⬇️ Coefficients CSV", coeff_df.to_csv(index=False),
                           "gwr_coefficients.csv", "text/csv")
    with col_b:
        st.download_button("⬇️ Residuals CSV",
                           gwr_df[["residual#number", "fitted#number"]].to_csv(index=False),
    ...
    streamlit_folium.st_folium(
        make_residual_map(_res_geojson, _geom_type, center), width=800, height=500)
    ...
    if st.session_state.local_I_arr is not None and "local_I#number" in gwr_df.columns:
        ...
        streamlit_folium.st_folium(
            make_lisa_map(_res_geojson, _geom_type, center), width=800, height=500)
...
if st.session_state.gwr_results is not None:
    gwr_df   = st.session_state.gwr_df
    ind_vars = st.session_state.independent_vars
    new_cols = (["Intercept#number"] + [f"{v}#number" for v in ind_vars] +
    ...
    selected_vars = st.multiselect("🧠 Select columns to include", available, default=available)
